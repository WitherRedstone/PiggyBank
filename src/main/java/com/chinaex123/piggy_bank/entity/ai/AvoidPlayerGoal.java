...
public class AvoidPlayerGoal extends Goal {
    ...
    private boolean isPlayerHoldingTameItem(Player player) {
        ...
        return mainHand.getItem() == tameItem || offHand.getItem() == tameItem;
    }
    ...
    public boolean tryTame(Player player) {
        ...
        if (mainHand.getItem() != tameItem && offHand.getItem() != tameItem) {
            return false;
        }

        // 消耗物品（无论成功失败都消耗）
        if (mainHand.getItem() == tameItem) {
            mainHand.shrink(1);
        } else if (offHand.getItem() == tameItem) {
            offHand.shrink(1);
        }
